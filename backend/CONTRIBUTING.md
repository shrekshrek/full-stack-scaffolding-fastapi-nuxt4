...
#### 依赖管理 (PDM)
- **强制使用**: **必须**使用 `PDM` 作为唯一的包和依赖管理器，以利用其现代化的解析和锁定机制。
- **配置文件**: 所有依赖项都定义在项目根目录的 `pyproject.toml` 文件中。
- **快捷命令**: 优先使用顶层 `package.json` 中定义的 `be:*` 脚本（如 `be:add`, `be:install`）来管理依赖，以遵循工作流护栏。
...
- **运行测试**: 在 `backend` 目录下，使用 `pdm run pytest` 命令来执行完整的测试套件
