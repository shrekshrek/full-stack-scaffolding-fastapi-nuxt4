<!-- eslint-disable vue/multi-word-component-names -->
<template>
  <div class="space-y-6">
    <!-- 页面标题 -->
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">创建权限</h1>
        <p class="text-gray-600 dark:text-gray-400 mt-1">创建新的系统权限</p>
      </div>
      
      <UButton
        icon="i-heroicons-arrow-left"
        variant="outline"
        size="sm"
        @click="navigateTo('/rbac/permissions')"
      >
        返回列表
      </UButton>
    </div>

    <!-- 创建表单 -->
    <UCard>
      <template #header>
        <h2 class="text-lg font-semibold">权限信息</h2>
      </template>

      <PermissionForm
        :loading="loading"
        :is-edit="false"
        @submit="handleSubmit"
        @cancel="handleCancel"
      />
    </UCard>
  </div>
</template>

<script setup lang="ts">
import type { PermissionCreate, PermissionUpdate } from '../../../types'
import { useRbacApi } from '../../../composables/useRbacApi'
import PermissionForm from '../../../components/PermissionForm.vue'

// 页面元数据
definePageMeta({
  title: '创建权限'
})

// 状态
const loading = ref(false)

// API
const rbacApi = useRbacApi()

// 处理表单提交
const handleSubmit = async (data: PermissionCreate | PermissionUpdate) => {
  loading.value = true
  try {
    await rbacApi.createPermission(data as PermissionCreate)
    
    // 显示成功消息
    const toast = useToast()
    toast.add({
      title: '创建成功',
      description: `权限 "${data.display_name}" 已创建`,
      color: 'success'
    })
    
    navigateTo('/rbac/permissions')
  } catch (error) {
    console.error('创建权限失败:', error)
    
    // 显示错误消息
    const toast = useToast()
    toast.add({
      title: '创建失败',
      description: '无法创建权限，请稍后重试',
      color: 'error'
    })
  } finally {
    loading.value = false
  }
}

// 处理取消
const handleCancel = () => {
  navigateTo('/rbac/permissions')
}
</script> 